...
const SWAP_TEST_1 = {
  TOKEN0_ADDR: "0x6B175474E89094C44Da98b954EedeAC495271d0F", // USDC
  TOKEN0_VAL: ethers.BigNumber.from("300"),
  TOKEN1_ADDR: "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48", // DAI Stable Coin
...
const createMetadataObject = (token0Address: string, token1Address: string, token0Amount: BigNumber, token1Amount: BigNumber) => {
  return {
...
describe("Uniswap Swaps Agent", () => {
  ...
  it("Should return a finding for a swap in Uniswap Pool", async () => {
    ...
    const swapFinding = createFinding(
      ...
      // for some reason, the pool meth
      createMetadataObject(SWAP_TEST_1.TOKEN0_ADDR, SWAP_TEST_1.TOKEN1_ADDR, SWAP_TEST_1.TOKEN0_VAL, SWAP_TEST_1.TOKEN1_VAL)
    );
  ...
  it("Should return 2 findings for 2 swaps in different Uniswap pools",
    async () => {
      ...
      const findings = await handleTransaction(mockTxEvent);
      const swapFinding1 = createFinding(
        SWAP_TEST_1.POOL_ADDR.toLocaleLowerCase(), 
        createMetadataObject(SWAP_TEST_1.TOKEN0_ADDR, SWAP_TEST_1.TOKEN1_ADDR, SWAP_TEST_1.TOKEN0_VAL, SWAP_TEST_1.TOKEN1_VAL)
      );

      const swapFinding2 = createFinding(
        SWAP_TEST_2.POOL_ADDR2.toLocaleLowerCase(),
        createMetadataObject(SWAP_TEST_2.TOKEN2_ADDR, SWAP_TEST_2.TOKEN3_ADDR, SWAP_TEST_2.TOKEN2_VAL, SWAP_TEST_2.TOKEN3_VAL)
      );
